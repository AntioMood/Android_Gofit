...
import com.example.gofit.adapter.DepositKelasAdapter
...
class DepositKelasActivity : AppCompatActivity() {
    ...
    private fun allDepositKelas(id_member: String){
        ...
        val stringRequest: StringRequest = object :
            StringRequest(Method.GET, MemberAPI.GET_DEPOK_BY_ID + id_member, Response.Listener { response ->
                val gson = Gson()
                val jsonObject = JSONObject(response)
                val depositKelas = gson.fromJson(
                    jsonObject.getJSONArray("data").toString(), Array<ModelDepositKelas>::class.java
                )
